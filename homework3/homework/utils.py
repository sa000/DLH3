...
class SuperTuxDataset(Dataset):
    def __init__(self, dataset_path, transform=None):
        ...
        """
        #load the file and iterate
        lable_file = open(dataset_path+'/labels.csv', 'r')
        label_reader = csv.reader(lable_file, delimiter=',')
        ...
        for row in label_reader:
            num+=1
            image_name, label, track = row
            image = Image.open(dataset_path+'/'+image_name)
            if self.transform:
                trans_im = self.transform(image)
                self.image_list.append([trans_im, LABEL_NAMES.index(label)])
            image_to_tensor = transforms.ToTensor()
            image_tensor = image_to_tensor(image)

            self.image_list.append([image_tensor, LABEL_NAMES.index(label)])
